...
for ($i = 0; $i < sizeof($checkbox1); $i++) {
  $query = "SELECT sub_code, sem  from add_fst where sub_name = '$checkbox1[$i]'";
  $q1 = mysqli_query($con, $query);
  while ($row = mysqli_fetch_assoc($q1)) {
    $code = $row['sub_code'];
    $sem = $row['sem'];
    $query1 = "INSERT INTO sub_reg (usn, sub_name, sub_code, sem) VALUES ('$user','$checkbox1[$i]','$code','$semester')";
...
?>
  ...
  require_once('../connection.php');
  $user = $_SESSION['user'];
  $q = mysqli_query($con, "SELECT * FROM add_fst");
  ...
  <body>
    <h1 class="myh1">Fastrack Subjects</h1>
    <div class="forms">
